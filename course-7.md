...
- 搭一个框架出来。可以用给的demo，也可以自己搓。我个人不太喜欢demo，课后我会给你们代码鉴赏。
...
需要一个方式来统一管理文字和按钮（组件），并且给文字和按钮排版，于是有了各种`JPanel`和各种Layout

需要以各种方式接受用户输入，所以有各种Listener

前三条要说的只有JFrame里自带一个JPanel，用getContentPane()来获取。剩下的就只有想到什么需求就去查对应的方法了。
...
其他的需求都可以用布局管理器解决，如果你不会解决，问我。

上面提到的两个，看https://blog.froster.icu/SUSTechCS109ProjectGuide 。翻到最下面。
...
Listener的本质：某件事发生时，调用绑定的方法。你们一般只会用到两个：
...
这种写法的理念是“把方法存在变量里，调用时从这个变量拿出方法来执行”
